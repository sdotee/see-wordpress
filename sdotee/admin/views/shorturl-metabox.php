...
$short_url = get_post_meta( $post->ID, '_sdotee_short_url', true );
$slug      = get_post_meta( $post->ID, '_sdotee_short_slug', true );
$domain    = get_post_meta( $post->ID, '_sdotee_short_domain', true );
$domains   = SDOTEE_Helpers::get_domains();
$default   = get_option( 'sdotee_default_domain', '' );
?>
	...
	<?php if ( ! empty( $short_url ) ) : ?>
		<div class="sdotee-shorturl-result">
			<p class="sdotee-shorturl-display">
				<a href="<?php echo esc_url( $short_url ); ?>" target="_blank" class="sdotee-short-url-link">
					<?php echo esc_html( $short_url ); ?>
				</a>
			</p>
			<div class="sdotee-shorturl-actions">
				<button type="button" class="button button-small sdotee-copy-btn" data-url="<?php echo esc_attr( $short_url ); ?>">
