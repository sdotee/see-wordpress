...
class SDOTEE_Helpers {
	...
	public static function get_api_key(): string {
		...
		if ( defined( 'SDOTEE_API_KEY' ) && ! empty( SDOTEE_API_KEY ) ) {
			return SDOTEE_API_KEY;
		}

		$encrypted = get_option( 'sdotee_api_key', '' );
		if ( empty( $encrypted ) ) {
			return '';
		}
	...
	public static function save_api_key( string $api_key ): void {
		if ( empty( $api_key ) ) {
			update_option( 'sdotee_api_key', '' );
			return;
		}
		update_option( 'sdotee_api_key', self::encrypt( $api_key ) );
	}
	...
	public static function get_api_base_url(): string {
		if ( defined( 'SDOTEE_API_BASE_URL' ) && ! empty( SDOTEE_API_BASE_URL ) ) {
			return SDOTEE_API_BASE_URL;
		}
		$url = get_option( 'sdotee_api_base_url', 'https://s.ee/api/v1/' );
	...
	private static function get_encryption_key(): string {
		$salt = defined( 'AUTH_KEY' ) ? AUTH_KEY : 'sdotee-default-key';
	...
	public static function get_domains( bool $force_refresh = false ): array {
		if ( ! $force_refresh ) {
			$cached = get_transient( 'sdotee_domains_cache' );
			if ( false !== $cached ) {
				return $cached;
			}
		}
		...
		try {
			...
			set_transient( 'sdotee_domains_cache', $domains, DAY_IN_SECONDS );
			return $domains;
	...
	public static function get_file_domains( bool $force_refresh = false ): array {
		if ( ! $force_refresh ) {
			$cached = get_transient( 'sdotee_file_domains_cache' );
			if ( false !== $cached ) {
				return $cached;
			}
		}
		...
		try {
			...
			set_transient( 'sdotee_file_domains_cache', $domains, DAY_IN_SECONDS );
			return $domains;
		} catch ( \See\Exception\SeeException $e ) {
	...
	public static function get_text_domains( bool $force_refresh = false ): array {
		if ( ! $force_refresh ) {
			$cached = get_transient( 'sdotee_text_domains_cache' );
			if ( false !== $cached ) {
				return $cached;
			}
		}
		...
		set_transient( 'sdotee_text_domains_cache', $domains, DAY_IN_SECONDS );
		return $domains;
	}
	 ...
	 *
	 * @param string $option_name Option name (sdotee_text_history or sdotee_file_history).
